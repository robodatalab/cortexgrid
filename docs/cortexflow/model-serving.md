...
| Function | Purpose |
|----------|---------|
| `save_model(weights_dir, serve_app, family, suffix) -> SavedModel` | Upload the weights directory, bundle the `serve_app` class + its pip deps, register a new MLflow `ModelVersion`. Uses the current Experiment's `run_id`/`run_name`. |
...
`Deployment`: `family`, `suffix`, `run_name`, `url`, `status`. cortexflow returns this handle and no more; the caller builds whatever HTTP client the serve-app's routes need.
...
caller (laptop / arc-runner / training job)
  |
  | cortexflow.save_model(weights_dir, ServeApp, family, suffix)
  |   1. upload weights_dir as-is to s3://<bucket>/models/<run_name>/<family>/<suffix>/weights/
  |   2. bundle_class(ServeApp) -> zip to s3://<bucket>/serve-bundles/<run_name>/<family>__<suffix>.zip
