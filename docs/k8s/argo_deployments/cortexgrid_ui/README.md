...
- **Reflector** ([../secrets/](../../../../k8s/argo_deployments/aws/secrets/)) - mirrors `aws-creds` (real AWS keys, used by cortexgrid.secrets to reach AWS Secrets Manager) and `ghcr-pull` (to pull the private images) into this namespace.
- **External Secrets Operator** ([../secrets/](../../../../k8s/argo_deployments/aws/secrets/)) - materializes `aws-creds` from `robolab/infra/AWS_*`. Service URLs and bucket names are read by the backend via cortexgrid library calls into SM, not pre-rendered into env.
