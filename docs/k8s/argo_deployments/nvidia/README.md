...
The chart's default `affinity` requires NFD (Node Feature Discovery) labels like `feature.node.kubernetes.io/pci-10de.present` to find GPU nodes. k3s doesn't ship NFD, so every node looks GPU-less and the DaemonSet schedules zero pods. We override `affinity` with a permissive `kubernetes.io/os: linux` match and restrict the DaemonSet to `nodeSelector: gpu=true` — the label the seed's `ComputeLabels` applies when `nvidia-smi -L` on the host lists a GPU — so it never lands on CPU-only nodes. (Passing `affinity: {}` doesn't work — Helm deep-merges the empty map into the default rather than replacing it.)
...
- **Ray** ([../ray/](../../../../k8s/argo_deployments/aws/ray/)) — requests `nvidia.com/gpu` in its Deployment, which wouldn't schedule without this plugin.
