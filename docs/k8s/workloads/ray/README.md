...
How the Ray head runs inside Kubernetes. Applied by the Argo Application at [../../argo-deployments/ray/](../../../../k8s/argo-deployments/ray/). See that folder's README for the higher-level "why raw manifests and not KubeRay" rationale.
...
- **`runtimeClassName: nvidia`** — runs the container via `nvidia-container-runtime` so the NVIDIA driver libraries + `nvidia-smi` are mounted in. Requires the [NVIDIA device plugin](../../../../k8s/argo-deployments/nvidia/).
