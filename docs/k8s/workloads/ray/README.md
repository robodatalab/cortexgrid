...
- **The same VRAM as a node label** — the worker also passes `--labels="vram_mib=<MiB>"`. The resource is consumable and shrinks as replicas take it; the label is static and names the card's size class. Ranking needs the second: `deploy_model` offers a replica the smallest class it fits before any larger one ([model-serving.md](../../../cortexgrid/model-serving.md#which-gpu-it-picks-when-several-fit)). A test asserts the two always carry the same number, since a replica offered a class the reservation then refuses would never be placed.
