    ...
    <div className="experiment-tree">
      ...
      <div className="experiment-tree__list">
        ...
        {experimentNames.map((name) => {
          ...
          const isExpLeaf = isExpSel && selection?.kind === 'experiment'
            ...
            <Fragment key={name}>
              ...
              {isExpSel && (
                <div className="experiment-tree__drawer">
                  {runs.length === 0 && (
                    <div className="experiment-tree__status">No runs</div>
                  )}
                  {runs.map((run) => {
                    const isRunSel = selectedRunId === run.run_id
                    const isRunLeaf = isRunSel && selection?.kind === 'run'
                    const runJobs = [...run.jobs].sort((a, b) =>
                      a.job_id.localeCompare(b.job_id),
                    )
                    return (
                      <Fragment key={run.run_id}>
                        <div
                          className={rowClass(isRunSel, isRunLeaf)}
                          onClick={() =>
                            onSelect({
                              kind: 'run',
                        ...
                        {isRunSel && (
                          <div className="experiment-tree__drawer">
                            {runJobs.length === 0 && (
                              <div className="experiment-tree__status">No jobs</div>
                            )}
                            {runJobs.map((job) => {
                              const isJobLeaf = selectedJobId === job.job_id
                              return (
                                <div
                                  key={job.job_id}
                                  className={rowClass(false, isJobLeaf)}
                                  onClick={() =>
                                    onSelect({
                                      kind: 'job',
                                      experiment_name: name,
