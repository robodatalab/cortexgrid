...
class TestRunEndpoints(unittest.TestCase):
    ...
    @patch(
        "cortexflow_ui.backend.main.list_run_metrics",
        return_value=["loss", "accuracy"],
    )
    ...
    def test_run_metric_history_passes_max_points_500(
        self, mock_history: MagicMock
    ) -> None:
        response = self.client.get("/api/runs/run-1/metrics/train/loss")
        self.assertEqual(response.status_code, 200)
        mock_history.assert_called_once_with("run-1", "train/loss", max_points=500)
    ...
    def test_run_url_returns_mlflow_url(self, _mock: MagicMock) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {"url": "http://test:5000/#/x"})
