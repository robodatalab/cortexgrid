...
def _log_params_dict() -> None:
    cortexgrid.log_params({"lr": "0.01"})


def _log_metrics_dict() -> None:
    cortexgrid.log_metrics({"loss": 0.5, "accuracy": 0.9, "lr": 0.01})


def _log_metric_history() -> None:
    cortexgrid.log_metric("loss", 1.0, step=0)
    cortexgrid.log_metric("loss", 0.5, step=1)
    cortexgrid.log_metric("loss", 0.25, step=2)
...
class TestExperiment(unittest.TestCase):
    ...
    def test_init_creates_experiment_visible_in_listings(self) -> None:
        ...
        self.assertIn(name, [e.experiment_name for e in cortexgrid.list_experiments()])
    ...
    def test_log_params(self, mode) -> None:
        ...
        self.assertEqual(cortexgrid.list_run_params(exp.run_id).get("lr"), "0.01")
    ...
    def test_log_metrics_dict(self, mode) -> None:
        ...
        metrics = cortexgrid.list_run_metrics(exp.run_id)
    ...
    def test_log_metric_history(self, mode) -> None:
        ...
        points = sorted(
            cortexgrid.get_metric_history(exp.run_id, "loss"), key=lambda p: p["step"]
        )
        self.assertEqual(
    ...
    def test_get_experiment_by_run_name_picks_correct_experiment_when_multiple_exist(
        ...
        self.addCleanup(cortexgrid.delete_experiment, name_a)
        self.addCleanup(cortexgrid.delete_experiment, name_b)
        exp_a = cortexgrid.Experiment.init(name_a)
        run_name_a = exp_a.run_name()
        cortexgrid.Experiment.close()
        cortexgrid.Experiment.init(name_b)
