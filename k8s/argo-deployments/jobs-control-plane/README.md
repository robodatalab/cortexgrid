...
- **Reflector** ([../secrets/](../secrets/)) — mirrors the cluster-wide `aws-creds` (seeded by `setup-node.py` in `external-secrets`) and `ghcr-pull` Secrets into this namespace. The Deployment consumes `aws-creds` via `envFrom` and references `ghcr-pull` in `imagePullSecrets`.
