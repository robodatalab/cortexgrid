.PHONY: setup-head setup-worker teardown-node
...
setup-head:
	...
	uv run python -m k8s.seed.setup-node --type=head --ip=$(IP) --storage-path=$(STORAGE_PATH) $(SSH_USER_FLAG)

setup-worker:
	@[ -n "$(IP)" ] || { echo "IP is required (e.g. make setup-worker IP=100.80.27.32)"; exit 1; }
	uv run python -m k8s.seed.setup-node --type=worker --ip=$(IP) $(SSH_USER_FLAG)

teardown-node:
	@[ -n "$(IP)" ] || { echo "IP is required (e.g. make teardown-node IP=100.80.27.32)"; exit 1; }
	uv run python -m k8s.seed.teardown-node --ip=$(IP) $(SSH_USER_FLAG)
